...
router.post('/',function(req, res){
	...
	db.collection('Users').find({ email : emailID }).toArray( function (err, docs){
		if(docs[0]){
			...
			db.collection('Users').insert({ "userName" : userNm, "email" : emailID, "gender" : gen, "age" : 0, "DPPath" : server_add+'/profile/Profile-Icon.png', "DPPathLow" : server_add+'/profile_low/Profile-Icon.png', "DPPathHigh" : server_add+'/profile/Profile-Icon.png', "status" : accStat }, function (err, docs){
				db.collection('Users').find({ email : emailID }).toArray( function (err, docs){
					res.header("Content-Type:","application/json");
					res.send(docs[0]);		
				});	
